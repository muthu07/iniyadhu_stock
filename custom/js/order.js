...
$(document).ready(function() {
	...
	$('#clientId').typeahead({
      name: 'clientname',
      remote: './php_action/clientname.php?query=%QUERY'
  }).bind('typeahead:selected', function(obj, selected, name) {
	//	if (selected.value.toString().split(',').length > 1) { /* "name - email@email.com" */
        //$(this).val(selected.value.toString().split(',')[0].replace(/^\s+|\s+$/g, ''));
  //  }
		$(this).val(selected.label);
		$("#clientContact").val(selected.phone);
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientId == "") {
				$("#clientId").after('<p class="text-danger"> The Client ID field is required </p>');
				$('#clientId').closest('.form-group').addClass('has-error');
			} else {
				$('#clientId').closest('.form-group').addClass('has-success');
			} // /else
	   	...
	   	for (var x = 0; x < quantity.length; x++) {
			...
			if(orderDate && clientId && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
